...
def p_expr_or(p):
	...
	if len(p) == 2:
		...
		p[0] = M_app(M_or(p.lineno(2)), [p[1],p[3]],p.lineno(2))

def p_addop(p):
	'''addop : ADD 
	| SUB'''
	# print "saw an addop on line",p.lineno(1)
	if p[1] == "+":
		p[0] = M_add(p.lineno(1))
	elif p[1] == "-":
		p[0] = M_sub(p.lineno(1))
...
def p_int_expr(p):
	...
	if len(p) == 2:
		...
		p[0] = M_app(p[2],[p[1],p[3]],p.lineno(2))
...
def p_mulop(p):
	'''mulop : MUL 
	         | DIV'''
	if p[1] == "*":
		p[0] = M_mul(p.lineno(1))
	elif p[1] == "/":
		p[0] = M_div(p.lineno(1))
...
def p_int_factor_ival(p):
	'''int_factor : IVAL'''
	p[0] = M_ival(p[1],p.lineno(1))
	
def p_int_factor_bval(p):
	'''int_factor : BVAL'''
	p[0] = M_bval(p[1],p.lineno(1))

def p_int_factor_sub(p):
	'''int_factor : SUB int_factor'''
	p[0] = M_app(M_neg(p.lineno(1)), [p[2]],p.lineno(1))

def p_bintterm_and(p):
	'''bint_term : bint_term AND bint_factor'''
	p[0] = M_app(M_and(p.lineno(2)), [p[1],p[3]],p.lineno(2))
...
def p_bint_factor_not(p):
	...
	p[0] = M_app(M_not(p.lineno(1)),[p[2]],p.lineno(1))
...
def p_int_factor_id(p):
	...
	if p[2] == None:
		p[0] = M_id(p[1],[],p.lineno(1))
	else:
		p[0] = M_app(M_fn(p[1],p.lineno(1)),p[2],p.lineno(1))
...
def p_prog_stmt(p):
	...
	if p[1] == "if":
		p[0] = M_cond(p[2],p[4],p[6],p.lineno(1))

	elif p[1] == "while":
		p[0] = M_while(p[2],p[4],p.lineno(1))
	
	elif p[1] == "read":
		p[0] = M_read(p[2],[],p.lineno(1)) #if we add arrays, this has to change.

	elif len(p) == 4: #this is filthy, but ASSIGN is the only one that fits
		p[0] = M_ass(p[1],[],p[3],p.lineno(1)) #will have to change for arrays

	elif p[1] == "print":
		p[0] = M_print(p[2],p.lineno(1))

def p_fun_body(p):
	'''fun_body : BEGIN prog_stmts RETURN expr SEMICOLON END'''
	p[0] = p[2] + [M_return(p[4],p.lineno(3))]

def p_fun_body_nobegin(p):
	'''fun_body : prog_stmts RETURN expr SEMICOLON'''
	p[0] = p[1] + [M_return(p[3],p.lineno(3))]

def p_type(p):
	'''type : INT
		| BOOL'''
	if p[1] == "bool":
		p[0] = M_bool(p.lineno(1))
	elif p[1] == "int":
		p[0] = M_int(p.lineno(1))
...
def p_var_declaration(p):
	...
	p[0] = M_var(p[2][0],p[2][1],p[2][2],p.lineno(1))
...
def p_fun_declaration(p):
	...
	p[0] = M_fun(p[2], p[3], p[5], p[7][0], p[7][1],p.lineno(1))
...
def p_prog(p):
	'''prog : block'''
	p[0] = M_prog(p[1].decls,p[1].stmts,p.lineno(1))

def p_block(p):
	'''block : declarations program_body'''
	p[0] = M_block(p[1],p[2],p.lineno(1))
