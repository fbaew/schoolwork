...
class M_fun(Node):
	...
	def collect(self,table):
		...
		for param in self.params:
			newTable.insert(param[0],param[2],"VAR",paramOffset)
			paramOffset -= 1
	...
	def check(self,table):
		...
		return I_FUN(table.lookup(self.name)["info"].symAttrib[0],funIR,varCount,len(self.params), stmtsIR)
...
class M_app(Node):
	...
	def check(self,table):
		...
		if type(self.oper) == M_fn:
			...
			return {"IR":I_APP(I_CALL(table.lookup(self.oper.name)["info"].symAttrib[0],table.lookup(self.oper.name)["level"]), exprIR ),
				"type":exprType}
		else:
			ops = {}
...
class M_fn(Node):
	...
	def check(self,table):
		return I_CALL(table.lookup(self.name)["info"].symAttrib[0],table.lookup(self.name)["level"]);
